...
<CodeBlock language="typescript">
  {`// Stock quote
const stockQuote = await fmp.quote.getQuote({ symbol: 'AAPL' });
...
<CodeBlock language="typescript">
  {`// Stock quotes
const stockQuote = await fmp.stock.getQuote({ symbol: 'AAPL' });
...
// Quote response type
const quote: Quote = await fmp.quote.getQuote({ symbol: 'AAPL' });
