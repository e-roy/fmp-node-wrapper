...
<CodeBlock language="typescript">
{`const marketCap = await fmp.stock.getMarketCap({ symbol: 'INVALID' });
...
<CodeBlock language="typescript">
{`// Invalid symbol
const result = await fmp.stock.getMarketCap({ symbol: 'INVALID_SYMBOL' });
// Result: { success: false, data: null, error: "No data found", status: 404 }

// API key issues
const result = await fmp.stock.getMarketCap({ symbol: 'AAPL' });
