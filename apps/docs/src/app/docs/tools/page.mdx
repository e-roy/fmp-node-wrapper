...
FMP Tools provides pre-built AI tools that can be used with various AI frameworks to integrate financial data into your AI applications. This package makes it easy to create chatbots, assistants, and AI-powered applications that can access real-time financial data.

## Installation

<CodeBlock language="bash">{`npm install fmp-ai-tools`}</CodeBlock>

<CodeBlock language="bash">{`pnpm add fmp-ai-tools`}</CodeBlock>

<CodeBlock language="bash">{`yarn add fmp-ai-tools`}</CodeBlock>
...
- [View API Documentation](/docs/api) - For direct API access
- [Check out Examples](/docs/examples) - See practical implementations
- [Visit GitHub Repository](https://github.com/e-roy/fmp-node-wrapper) - For source code and issues
