...
- **Node.js 20+** - Required for OpenAI Agents
- **@openai/agents** - Version `>=0.11.0` (optional peer dependency)
- **zod** - Version `^4.0.0` (required for schema validation)

## Installation

`@openai/agents` is an optional peer dependency, so install it alongside `fmp-ai-tools` and `zod`:

<CodeBlock language="bash">{`npm install fmp-ai-tools @openai/agents zod`}</CodeBlock>

<CodeBlock language="bash">{`pnpm add fmp-ai-tools @openai/agents zod`}</CodeBlock>

<CodeBlock language="bash">{`yarn add fmp-ai-tools @openai/agents zod`}</CodeBlock>
...
- `getHistoricalPrice` - Historical daily prices (most recent `limit` days, default 30)
- `getIntraday` - Intraday price bars at a given interval (most recent `limit` bars, default 50)
...
### 📰 News Tools
- `getStockNews` - Latest general stock market news (default `limit` 20)
- `getStockNewsBySymbol` - Latest news for one or more symbols (default `limit` 20)

### 🔎 Screener Tools
- `screenStocks` - Screen for stocks by market cap, price, sector, exchange, etc. (default `limit` 50)

### 🔍 Search Tools
- `searchSymbol` - Resolve a company name or partial ticker to matching symbols

### 🎯 Analyst Tools
- `getAnalystEstimates` - Analyst estimates (revenue, EBITDA, net income, EPS) by period
- `getPriceTargetConsensus` - Analyst price-target consensus
- `getStockGrades` - Recent analyst grades / upgrades & downgrades

### 🧮 Valuation Tools
- `getDiscountedCashFlow` - DCF fair value vs. current price
- `getCompanyRating` - Company rating/score snapshot

### 📐 Technical Tools
- `getTechnicalIndicator` - Indicator series (SMA, EMA, RSI, etc.) at a chosen timeframe
...
FMP Tools targets the current `@openai/agents` releases:

- ✅ **Supported**: `@openai/agents@>=0.11.0` (tested against `0.11.x`)
- ❌ **Not Supported**: `0.1.x` and other pre-`0.11` releases

### Zod Version Requirements

The OpenAI tool wrapper passes the Zod schema natively to the agents `tool()`, so a current `zod` is required:

- ✅ **Supported**: `zod@^4.0.0`
- ❌ **Not Supported**: `zod@3` and older
...
**Solution**: Ensure `@openai/agents` is `>=0.11.0` and `zod` is `^4.0.0`.
