...
import { quoteTools } from 'fmp-tools/vercel-ai';
...
const loadTools = async (category: string) => {
  switch (category) {
    case 'stocks':
      return await import('fmp-tools/vercel-ai').then(m => m.stockTools);
    case 'market':
      return await import('fmp-tools/vercel-ai').then(m => m.marketTools);
    default:
      return await import('fmp-tools/vercel-ai').then(m => m.fmpTools);
  }
};`}
</CodeBlock>
...
<CodeBlock language="typescript" filename="app/api/chat/route.ts">
  ...
  experimental_onToolCall: ({ toolCall }) => {
    ...
    analytics.track('tool_used', { tool: toolCall.toolName });
  },
});`}
</CodeBlock>
...
messages: convertToModelMessages(messages),
tools: fmpTools,
stopWhen: isProduction ? stepCountIs(3) : stepCountIs(5), // Stricter limits in production
temperature: isProduction ? 0.5 : 0.7, // More conservative in production
});`}
...
Implement health checks for your API routes:

<CodeBlock language="typescript" filename="app/api/health/route.ts">
  {`export async function GET() {
  try {
    // Test basic functionality
    const testResult = await fetch('https://api.example.com/health');
...
describe('Chat API', () => {
it('should handle stock quote requests', async () => {
