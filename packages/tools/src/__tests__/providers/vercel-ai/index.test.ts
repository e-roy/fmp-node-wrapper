...
const mockClient = {
  ...
  financial: {
    ...
    getKeyMetrics: jest.fn().mockResolvedValue({ data: [] }),
    getFinancialRatios: jest.fn().mockResolvedValue({ data: [] }),
    getEnterpriseValue: jest.fn().mockResolvedValue({ data: [] }),
    getCashflowGrowth: jest.fn().mockResolvedValue({ data: [] }),
    getIncomeGrowth: jest.fn().mockResolvedValue({ data: [] }),
    getBalanceSheetGrowth: jest.fn().mockResolvedValue({ data: [] }),
    getFinancialGrowth: jest.fn().mockResolvedValue({ data: [] }),
    getEarningsHistorical: jest.fn().mockResolvedValue({ data: [] }),
    getEarningsSurprises: jest.fn().mockResolvedValue({ data: [] }),
  },
  calendar: {
...
describe('Vercel AI Provider Index (minimal)', () => {
  ...
  it('executes all tools with minimal valid args', async () => {
    const minimalArgs: Record<string, any> = {
      ...
      getKeyMetrics: { symbol: 'AAPL', period: 'annual' },
      getFinancialRatios: { symbol: 'AAPL', period: 'annual' },
      getEnterpriseValue: { symbol: 'AAPL', period: 'annual' },
      getCashflowGrowth: { symbol: 'AAPL', period: 'annual' },
      getIncomeGrowth: { symbol: 'AAPL', period: 'annual' },
      getBalanceSheetGrowth: { symbol: 'AAPL', period: 'annual' },
      getFinancialGrowth: { symbol: 'AAPL', period: 'annual' },
      getEarningsHistorical: { symbol: 'AAPL' },
