...
describe('Financial Endpoints', () => {
  ...
  describe('getKeyMetrics', () => {
    ...
    it('should fetch key metrics for AMZN', async () => {
      ...
      expect(metrics.evToSales).toBeDefined();
    }, 15000);
  });
  ...
  describe('getFinancialRatios', () => {
    ...
    it('should fetch financial ratios for NVDA', async () => {
      ...
      expect(ratios.debtToEquityRatio).toBeDefined();
    }, 15000);
  });
  ...
  describe('Error handling and edge cases', () => {
    ...
    it('should handle different symbols consistently', async () => {
      ...
      results.forEach(result => {
        ...
        if (result.data!.length > 0) {
          ...
          expect(statement.symbol).toBeDefined();
          // Stable API might return different period formats, so just check it's defined
          expect(statement.period).toBeDefined();
          expect(typeof statement.period).toBe('string');
        }
    ...
    it('should validate data consistency across multiple calls', async () => {
      ...
      if (result1.data!.length > 0 && result2.data!.length > 0) {
        ...
        expect(statement1.period).toBeDefined();
        expect(statement2.period).toBeDefined();
        if (statement1.reportedCurrency && statement2.reportedCurrency) {
